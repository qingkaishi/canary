...
void cvector_resize(const cvector cv, const size_t len)
{
	if(cv->cv_len > len)
	{
		fprintf(stderr, "You will lose data if cvector is shortented (%zd -> %zd)\n", cv->cv_len, len);
	}
        cv->cv_pdata = realloc(cv->cv_pdata, len * cv->cv_size);
        cv->cv_len = len;
}
...
void cvector_clear(const cvector cv)
{
	...
	free(cv->cv_pdata);
	cv->cv_tot_len = MIN_LEN;
	cv->cv_pdata = malloc(MIN_LEN * cv->cv_size);
}
...
{
        return (void*)((long)cv->cv_pdata + index * cv->cv_size);
}
