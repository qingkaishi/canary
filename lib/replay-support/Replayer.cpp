...
void static inline load(int svId, int tid) {
    ...
    while (GLOG[svId][currentIdx] != tid) {
#ifdef DEBUG
        if (count++ < 2) {
    ...
    if (GLOG[svId][currentIdx] == tid && GLOG[svId][currentIdx + 1] > 0) {
        // do the event
...
extern "C" {
    ...
    void OnFork(int forked_tid_ptr) {
        if (!start) {
            return;
        }
    ...
    void OnWait(int condId, int cond_ptr, int mutex_ptr) {
        if (!start) {
            return;
        }
        ...
        while (GLOG[num_shared_vars - 2][currentIdx] != _tid) {
            //wait;
            //printf("#### %d\n", num_shared_vars - 2);
            struct timespec tv;
            tv.tv_sec = time(0);
        ...
        if (GLOG[num_shared_vars - 2][currentIdx] == _tid && GLOG[num_shared_vars - 2][currentIdx + 1] > 0) {
            // do the event
