...
static void* canary_heap_start = NULL;
static void* canary_heap_end = NULL;

void*
get_canary_heap_start(){
  return canary_heap_start;
}

void*
get_canary_heap_end(){
  return canary_heap_end;
}
...
public_mALLOc(size_t bytes)
{
  ...
  /* deal with canary_heap_start & canary_heap_end */
  if (victim != NULL) {
    size_t sptr = (size_t) victim;
    size_t eptr = (size_t) (sptr + bytes);

    assert(sptr <= eptr);

    if(canary_heap_start == NULL && canary_heap_end == NULL){
        canary_heap_start = victim;
        canary_heap_end = (void*) eptr;
    }

    if(sptr < (size_t) canary_heap_start) {
        canary_heap_start = victim;
    }

    if(eptr > (size_t) canary_heap_end) {
        canary_heap_end = (void*) eptr;
    }
  }
