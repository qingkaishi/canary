...
int Transformer4Dummy::stmt_idx = 0;

Transformer4Dummy::Transformer4Dummy(Module* m, set<Value*>* svs, unsigned psize) : Transformer(m, svs, psize) {
    int idx = 0;
...
void Transformer4Dummy::beforeTransform(AliasAnalysis& AA) {
...
void Transformer4Dummy::afterTransform(AliasAnalysis& AA) {
...
bool Transformer4Dummy::functionToTransform(Function* f) {
    return !f->isIntrinsic() && !f->empty()
            && ignored_funcs.find(f) == ignored_funcs.end()
            && !this->isInstrumentationFunction(f);
}

bool Transformer4Dummy::blockToTransform(BasicBlock* bb) {
    return true;
}

bool Transformer4Dummy::instructionToTransform(Instruction* ins) {
    return true;
}
...
void Transformer4Dummy::transformAddressInit(CallInst* inst, AliasAnalysis& AA){
    Value * val = inst;
...
void Transformer4Dummy::transformLoadInst(LoadInst* inst, AliasAnalysis& AA) {
...
void Transformer4Dummy::transformStoreInst(StoreInst* inst, AliasAnalysis& AA) {
...
void Transformer4Dummy::transformPthreadCreate(CallInst* ins, AliasAnalysis& AA) {
    ConstantInt* tmp = ConstantInt::get(BOOL_TY(module), 0);
    this->insertCallInstBefore(ins, F_prefork, tmp, NULL);
    this->insertCallInstAfter(ins, F_fork, ins->getArgOperand(0), tmp, NULL);
}

void Transformer4Dummy::transformPthreadMutexInit(CallInst* ins, AliasAnalysis& AA) {
    ConstantInt* tmp = ConstantInt::get(BOOL_TY(module), 0);
...
void Transformer4Dummy::transformPthreadMutexLock(CallInst* ins, AliasAnalysis& AA) {
...
void Transformer4Dummy::transformPthreadCondWait(CallInst* ins, AliasAnalysis& AA) {
    Value * val = ins->getArgOperand(0);
    int svIdx = this->getValueIndex(val, AA);
    if (svIdx == -1) return;
...
void Transformer4Dummy::transformPthreadCondTimeWait(CallInst* ins, AliasAnalysis& AA) {
    Value * val = ins->getArgOperand(0);
    int svIdx = this->getValueIndex(val, AA);
    if (svIdx == -1) return;
...
void Transformer4Dummy::transformSystemExit(CallInst* ins, AliasAnalysis& AA) {
    this->insertCallInstBefore(ins, F_exit, NULL);
}

bool Transformer4Dummy::isInstrumentationFunction(Function * called) {
...
int Transformer4Dummy::getValueIndex(Value* v, AliasAnalysis & AA) {
    set<Value*>::iterator it = sharedVariables->begin();
    while (it != sharedVariables->end()) {
        Value * rep = *it;
